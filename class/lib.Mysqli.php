...
abstract class db {
    ...
    public static function connect($host, $user, $pass, $dbname, $port = null) {
        ...
        if (self::$mysqli->connect_errno) {
            show404(" Connect to MySQL server " , self::$mysqli->connect_error);
        }
    ...
    public static function setCharset($charset) {
        ...
        if (self::$mysqli->errno) {
            show404(" Database error " , self::$mysqli->error);
        }
    ...
    public static function escapeString($str) {
        ...
        if (self::$mysqli === null) {
           show404(" Database error" , "Server is disconnected ");
        }
    ...
    /**
     * recursive escape items,
     * return escaped separated string
     */

    private static function escapeRecursive($arr) {

        foreach ($arr as $k => $item) {
            $arr[$k] = "'" . self::$mysqli->real_escape_string($item) . "'";
        }
    ...
    private static function buildQuery() {
        ...
        foreach ($args as $key => $arg) {
            ...
            if ($key == 0) {
                ...
                $params[] = is_array($arg) ? self::escapeRecursive($arg) : self::$mysqli->real_escape_string($arg);
            }

        }

        if (!isset($query)) {
            show404(" Database error" , "Query is empty ");
        }
    ...
    private static function sendSetQuery($queryString) {
        ...
        if (self::$mysqli->errno) {
            show404(" Database error " , self::$mysqli->error . " ::: {$queryString}");
        }
